...
def matches():
    ...
    try:
        ...
        # debug
        raw = db.execute(text("SELECT * FROM fantasy_matchups")).fetchall()
        print(f"[matches] raw matchups: {len(raw)}")
        for r in raw:
            print(f"  {r}")
        mgrs = db.execute(text("SELECT id, team_name FROM managers")).fetchall()
        print(f"[matches] managers: {[(m.id, m.team_name) for m in mgrs]}")
        ...
        """)).fetchall()
        print(f"[matches] after join: {len(matchups)}")
