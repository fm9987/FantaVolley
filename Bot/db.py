...
def generate_schedule(manager_ids: list[int]) -> dict[int, list[tuple[int,int]]]:
    """
    Generate a round-robin schedule for n managers.
    Returns {gameweek: [(home_id, away_id), ...]}
    Works for any even number of teams.
    """
    n = len(manager_ids)
    if n % 2 != 0:
        manager_ids.append(None)  # bye week if odd number
        n += 1

    teams = manager_ids.copy()
    rounds = {}
